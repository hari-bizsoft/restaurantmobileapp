...
import com.bizsoft.restaurant.adapters.FileMangerAdpater;
import com.bizsoft.restaurant.dataobjects.BizFile;
...
public class InvoiceListActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        BizUtils.addCustomActionBar(InvoiceListActivity.this,"Invoice List");
        BizUtils.addCustomActionBar(InvoiceListActivity.this,"Invoice/Reports");
        String path = Environment.getExternalStorageDirectory() +
                File.separator + "fmcg";
        Log.d("Files", "Path: " + path);


        String pathforReports = Environment.getExternalStorageDirectory() +
                File.separator + "reports";
        Log.d("Files", "reports Path: " + pathforReports);


        File directory = new File(path);
        File Reportsdirectory = new File(pathforReports);


        final File[] files = directory.listFiles();
        final File[] filesReports = Reportsdirectory.listFiles();
        ...
        if(filesReports!=null) {

            Log.d("Files", "Size: " + filesReports.length);


            for (int i = 0; i < filesReports.length; i++) {
                BizFile bizFile = new BizFile();
                bizFile.setFileName(filesReports[i].getName());
                bizFile.setFileSize(filesReports[i].getUsableSpace());
                bizFile.setDetails(filesReports[i]);
                bizFile.setDate(String.valueOf(filesReports[i].lastModified()));
                fileList.add(bizFile);
                allFileList.add(bizFile);
                Log.d("Files", "FileName:" + filesReports[i].getName());
            }
        }
        else
        {
            Toast.makeText(this, "No report record found...", Toast.LENGTH_SHORT).show();
        }
